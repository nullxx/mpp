...
void read_file(const char *path, RawSentenceT *raw_sentence_t) {
    ...
    while (fgets(line_buffer, MAX_FILE_LINE_SIZE, fp)) {
        ...
        RawSentence *raw_sentences_rllc = realloc(raw_sentences, sizeof(RawSentence) * (rs->row + 1));  // FIXME raw_sentences nulled but not freed
        if (raw_sentences_rllc == NULL) {
            err.type = FATAL;
            err.message = "Error reading file";
            goto error;
        }
        raw_sentences = raw_sentences_rllc;
