...
CheckParamsR check_arithmetic_params(TokenName token_id_name, Sentence *st) {
    ...
    for (unsigned int i = 0; i < sequence_templates_count; i++) {
        ...
        if (sequence_templates[i].paramT->count != st->tokens_count - 1) {
			...
			const int size = sizeof(char) * (strlen(message) + expected_len + found_len + strlen(sequence_templates[i].token->content) - 2 * 3) + 1;
            cpr.message = malloc(size);
            snprintf(cpr.message, size, message, sequence_templates[i].paramT->count, st->tokens_count - 1, sequence_templates[i].token->content);
            return cpr;
        }
    }
