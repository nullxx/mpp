...
CheckParamsR check_params(TokenName token_id_name, const Sentence *st) {
    CheckParamsR cpr = check_arithmetic_params(token_id_name, st);
...
Token *get_tokens(RawSentence *rs, int *count) {
    ...
    unsigned i = 0;
    while (token != NULL) {
        Token *tokens_rllc = realloc(tokens, sizeof(Token) * (i + 1));  // FIXME tokens nulled but not freed
        if (tokens_rllc == NULL) {
            free(tokens);
            return NULL;
        }
...
Sentence process_line(RawSentence *rs) {
    ...
    if (token_id == NULL) {
        ...
        const int size = sizeof(char) * (detail_message_len + row_num_len - 2 * 1) + 1;
        char *message = malloc(size);
        snprintf(message, size, detail_message, rs->row + 1);
    ...
    if (sen_type == NO_SENTENCE_TYPE) {
		...
		const int size = sizeof(char) * (detail_message_len + row_num_len + token_len - 2 * 2) + 1;
        char *message = malloc(size);

        snprintf(message, size, detail_message, token_id->content, rs->row + 1);
